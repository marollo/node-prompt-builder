...
30. ✅ Claude image-to-text — image + node-specific system prompt sent to Claude; system prompts in `src/prompts/*.md`; `claudeClient.js` handles the API call; Describe button on image slots removed pending UX redesign
...
36. ✅ Image node aspect ratio — `_buildImageEl` now stores `_aspectRatio` from `img.naturalWidth / img.naturalHeight` in an `onload` callback; `computeSize` and `onDrawForeground` both use this ratio so the thumbnail fills the node width with correct proportions and no cropping (`ImageNode.js`)
37. ✅ Claude node (model category) — `ClaudeNode.js`; image input + text output; Type dropdown maps to five `.md` system prompts; Describe button calls `claudeClient.describeImage()`; result drawn in canvas text box with word-wrap; `getPromptFragment()` added so output wires into Prompt Assembler; registered as `model/Claude`
38. ✅ Remove Claude cost bar from content nodes — `claudeSpent`, `claudeCallCount`, `addClaudeStatsDrawing()` import and call removed from all five content nodes (`SubjectNode.js`, `LocationNode.js`, `CameraNode.js`, `LightingNode.js`, `StyleMoodNode.js`)
39. ✅ Camera Move node — `CameraMoveNode.js` + `src/api/formats/replicate.js`; image input/output; three canvas-drawn control rows (orbit/zoom/tilt) with click handling via `onMouseDown`; Generate button calls Replicate `qwen/qwen-edit-multiangle`; result thumbnail drawn proportionally; registered as `model/CameraMove`
40. ✅ IndexedDB large-value fix — `storageUtils.js` updated to store the graph as a `Blob` (JSON → Blob on save, Blob → JSON on load); bypasses Chrome's "Failed to read large IndexedDB value" bug that fires when base64 image data pushes the stored object above ~20MB; backward compatible with old plain-object saves
41. ✅ Replicate CORS fix — `vite.config.js` created with a dev server proxy; `/api/replicate/*` forwarded to `https://api.replicate.com/v1/*` server-side; `REPLICATE_API_BASE` in `replicate.js` changed from the full Replicate URL to `/api/replicate`
